...
class AIService:
    ...
    def __init__(self):
        ...
        self.client = self._build_client(GROQ_API_KEY)
    ...
    def _build_client(self, api_key: Optional[str]) -> Optional[OpenAI]:
        normalized_key = self._normalize_api_key(api_key)
        if not USE_REAL_AI or not normalized_key:
            return None
        return OpenAI(api_key=normalized_key, base_url=GROQ_BASE_URL)

    def _get_client(self, api_key_override: Optional[str] = None) -> Optional[OpenAI]:
        override_key = self._normalize_api_key(api_key_override)
        if override_key:
            return self._build_client(override_key)
        return self.client
